...
factory/research/   fuentes de datos + scorer + pipeline diario
factory/script/     guiones (hito 2)        factory/media/      TTS, imágenes, stock (hito 3)
factory/video/      montaje FFmpeg (h3)     factory/thumbnail/  miniaturas (h3)
factory/seo/        títulos, tags (h4)      factory/publish/    subida a YouTube (h4)
factory/analytics/  métricas y cerebro (h5) factory/web/        dashboard
```
...
**Base de datos**
- Toda transacción que vaya a escribir usa **`BEGIN IMMEDIATE`**. El `DEFERRED` por defecto falla
  al instante con "database is locked" ignorando el `busy_timeout`, y subirlo no lo arregla nunca.
